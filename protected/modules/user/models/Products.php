...
class Products extends CActiveRecord
{
	...
	public static function getProductList(){
        ...
        ->select('id,title')
        ->from('products')
        ->where('active=:active', array(':active'=>1)) //where($condition, $params)   
        ->queryAll();   		
	}

	public static function getCartData(){
		if(empty(Yii::app()->session['cart_item']))
				return array();
		return Yii::app()->db->createCommand()
        ->select('id,title,price')
        ->from('products')
        ->where(array('in', 'id', Yii::app()->session['cart_item'])) //where($condition, $params)   
        ->queryAll();   		
	}
}
